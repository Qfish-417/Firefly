...
- 已完成 digest-bound lexical 固定评测：ACL/Recall/Citation Probe 读取同一 `IndexEvaluationSet`，专用 PostgreSQL evaluator 只访问任务绑定的 building 版本并使用真实 ACL，激活后不可继续读取。vector/hybrid 固定评测仍待实现。
- 下一步补 reconciliation 定时调度和 retired 版本回收，再扩展 vector/hybrid 评测与 PDF/代码/表格 Chunker，最后接生产 BM25/ANN。
...
| M5 记忆工具（进行中） | 授权检索、聚合、版本索引、质量报告、后台 Worker、删除 Ack、Parent/Child 扩展 | 索引构建/激活、digest-bound lexical 质量评测、Markdown Parent/Child 和 MinIO 删除恢复已通过；vector/hybrid 评测、其他 Chunker、调度器、旧版 GC、BM25/ANN 与多模态待补齐 |
