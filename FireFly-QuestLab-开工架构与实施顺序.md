...
- 下一步实现 PDF/代码/表格 Chunker，再接生产 BM25/ANN 和外部索引回收。
...
| M5 记忆工具（进行中） | 授权检索、聚合、版本索引、质量报告、后台 Worker、删除 Ack、Parent/Child 扩展 | 索引构建/激活、digest-bound lexical/vector/hybrid 质量评测、Markdown Parent/Child、MinIO 删除恢复、周期 reconciliation 与本地旧版 GC 已通过；其他 Chunker、BM25/ANN、外部索引回收与多模态待补齐 |
