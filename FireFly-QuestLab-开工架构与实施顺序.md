...
- M5.8/M5.9/M5.10 已实现 PDF Layout、代码 AST、表格和对话结构化 Chunker、`ParserBackedIndexSourcePort` 以及严格/降级策略；M5.11 已接入确定性关系扩展和 PostgreSQL 候选源；M5.12 已增加 reconciliation/retired-index GC 持久化维护账本；M5.13 已提供受治理 HTTP Parser Adapter；M5.14 已落地真实 TypeScript/JavaScript AST Parser；M5.15 已落地真实 CSV 单表 Parser；M5.16 已落地版本化转录 JSON Parser。下一步部署 PDF/OCR、XLSX、真实 ASR/说话人分离引擎，再接生产 BM25/ANN 和外部索引回收。
...
| M5 记忆工具（进行中） | 授权检索、聚合、版本索引、质量报告、后台 Worker、删除 Ack、Parent/Child 与关系扩展 | 索引构建/激活、digest-bound lexical/vector/hybrid 质量评测、Markdown/PDF/代码/表格/对话 Parent/Child、Parser SourcePort 边界、TS/JS AST、CSV 与版本化转录 JSON Parser、PostgreSQL 关系候选源、维护周期持久化账本、MinIO 删除恢复、周期 reconciliation 与本地旧版 GC 已通过；PDF/OCR/XLSX/真实 ASR Provider、BM25/ANN、外部索引回收与多模态待补齐 |
