...
- M5.8/M5.9/M5.10 已实现 PDF Layout、代码 AST、表格和对话结构化 Chunker、`ParserBackedIndexSourcePort` 以及严格/降级策略；M5.11 已接入确定性关系扩展和 PostgreSQL 候选源。下一步接入真实 parser Provider，再接生产 BM25/ANN 和外部索引回收。
...
| M5 记忆工具（进行中） | 授权检索、聚合、版本索引、质量报告、后台 Worker、删除 Ack、Parent/Child 与关系扩展 | 索引构建/激活、digest-bound lexical/vector/hybrid 质量评测、Markdown/PDF/代码/表格/对话 Parent/Child、Parser SourcePort 边界、PostgreSQL 关系候选源、MinIO 删除恢复、周期 reconciliation 与本地旧版 GC 已通过；真实 parser Provider、BM25/ANN、外部索引回收与多模态待补齐 |
