...
1. 为 `reconcileFailedDeletionTargets` 增加可观测的周期调度器，并逐个实现 lexical/vector/cache/summary/evaluation 删除 Provider 与证据核验。
2. 为 retired 版本建立带保留期的垃圾回收任务，删除必须避开 active 和仍被审计引用的版本。
