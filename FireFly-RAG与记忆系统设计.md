...
- M5.13 已提供固定 HTTPS Endpoint、source-type allowlist、超时、响应大小和版本化 JSON 校验的 `HttpIndexSourceParser`；M5.14 已使用官方 TypeScript Compiler API 落地 TS/TSX/JS/JSX 真实 AST Parser；M5.15 已使用 `csv-parse` 落地严格 CSV 单表 Parser；M5.16 已落地版本化转录 JSON Parser。下一批部署并接入真实 PDF/OCR、XLSX 工作簿、ASR 与说话人分离引擎，再接生产 BM25/ANN Provider 与外部索引回收。
